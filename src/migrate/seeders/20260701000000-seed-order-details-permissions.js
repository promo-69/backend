...
module.exports = {
		...
		await queryInterface.sequelize.transaction(async (transaction) => {
			...
			await queryInterface.bulkInsert(
				...
				[
					{
						code: 'ORDER-DETAILS',
						description: 'Detalle de órdenes para validación de QR (boletos y confitería)',
					},
				],
			...
			// 4. Asignar a los roles operativos (READ + UPDATE de ORDER-DETAILS)
	...
	async down(queryInterface) {
		await queryInterface.sequelize.transaction(async (transaction) => {
         ...
         WHERE resource IN (SELECT id FROM resources WHERE code = 'ORDER-DETAILS')`,
				{ transaction },
			);

			await queryInterface.sequelize.query(`DELETE FROM resources WHERE code = 'ORDER-DETAILS'`, {
				transaction,
			});
		});
	},
