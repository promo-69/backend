...
export class ReportsManagementService {
	...
	async getRentalsReport(cinemaId?: number, filters: { from?: string; to?: string } = {}) {
		...
		const rentalWhere: any = { requested_start_time: { [Ops.between]: [from, to] } };
		if (cinemaId) rentalWhere.cinema = cinemaId;
		...
		const rentalsRaw = await this._rentalRequests.getAll(
			{
				...
				relations: [
					...
					{ association: '_Rooms', attributes: ['id', 'name'], required: false },
