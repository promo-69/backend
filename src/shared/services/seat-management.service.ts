...
export class SeatManagementService {
	...
	async createSeats(
		...
		if (!Array.isArray(seats) || seats.length === 0) throw new ValidationError('Debe proporcionar al menos un asiento');
		...
		return this._seats.transaction(async (transaction: Transaction) => {
			const conditions = seatRecords.map((record) => ({
				row_identifier: record.row_identifier,
				column_number: record.column_number,
			}));

			const existing = await this._seats.getAll(
				{ count: false },
				{
					room: roomId,
					[Op.or]: conditions,
			...
			if (existingRows.length > 0) {
				...
				throw new ConflictError(
					`Ya existe un asiento en la posición ${firstExisting.row_identifier}${firstExisting.column_number}`,
					'SEAT_POSITION_DUPLICATE',
				);
			}

			const createdSeats = await this._seats.bulkCreate(seatRecords, { transaction });
