...
export class ShowtimeManagementService {
	...
	// -------------------------------------------------------------------------
	//  HELPERS DE CICLO DE VIDA (sin cambios respecto al original)
	// -------------------------------------------------------------------------
	...
	private async _getLifecycleStateIds(): Promise<Record<string, number>> {
		...
		for (const s of stateList) map[s.description] = s.id;
		return map;
	}

	// -------------------------------------------------------------------------
	//  VALIDACIÓN DE SOLAPAMIENTO (sin cambios)
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	//  CICLO DE VIDA - PELÍCULAS (sin cambios respecto al original)
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	//  CICLO DE VIDA - EVENTOS ESPECIALES (misma lógica que películas)
	// -------------------------------------------------------------------------
	private async _syncEventLifecycle(eventId: number, transaction?: Transaction) {
		...
		// Si el admin lo pasó manualmente a Últimos Días, respetamos su decisión
	...
	// -------------------------------------------------------------------------
	//  CARTELERA PÚBLICA - PELÍCULAS (sin cambios respecto al original)
	// -------------------------------------------------------------------------
	async getBillboard(filters?: any) {
		...
		if (!bookingList.length) {
			if (targetCinemaId) throw new NotFoundError('No se encontraron funciones programadas para la sucursal seleccionada');
			return { count: 0, rows: [] };
		}
		...
		if (!bookingList.length) throw new NotFoundError('No se encontraron funciones programadas para la sucursal seleccionada');
		...
		// Solo showtimes de películas (movie NOT NULL)
		const showtimeWhere: any = { booking: bookingIds, deleted_at: null, movie: { [Ops.ne]: null } };
		...
		if (filters && typeof filters === 'object') {
			...
			if (filters.projectionType) {
				...
				showtimeList = showtimeList.filter((s: any) => {
					...
					return desc.includes(pt) || String(s.projection_type) === pt;
				});
			}
			if (filters.language) {
				...
				showtimeList = showtimeList.filter((s: any) => {
					...
					return desc.includes(lang) || String(s.language) === lang;
				});
			}
		}

		if (!showtimeList.length) throw new NotFoundError('No se encontraron funciones que coincidan con los filtros seleccionados para esta sucursal');
		...
		const [projTypes, languages, currencies] = await Promise.all([
			(Database.repository('main', 'projection-types') as any).getAll({ count: false, attributes: ['id', 'description'] }, { id: projTypeIds }),
			(Database.repository('main', 'languages') as any).getAll({ count: false, attributes: ['id', 'description'] }, { id: languageIds }),
			(Database.repository('main', 'currencies') as any).getAll({ count: false, attributes: ['id', 'code', 'symbol'] }, { id: currencyIds }),
		]);

		const projMap = new Map<number, any>((Array.isArray(projTypes) ? projTypes : projTypes.rows).map((p: any) => [p.id, p]));
		const langMap = new Map<number, any>((Array.isArray(languages) ? languages : languages.rows).map((l: any) => [l.id, l]));
		const currMap = new Map<number, any>((Array.isArray(currencies) ? currencies : currencies.rows).map((c: any) => [c.id, c]));
		...
		for (const s of showtimeList) {
			...
			if (!billboardMap.has(movie.id)) {
				billboardMap.set(movie.id, {
					movie: {
						...
						lifecycle: movie._LifecycleStates ? { id: movie._LifecycleStates.id, description: movie._LifecycleStates.description } : null,
						age_classification: movie._AgeClassifications ? { id: movie._AgeClassifications.id, description: movie._AgeClassifications.description } : null,
						genres: movie._Genres ? movie._Genres.map((g: any) => ({ id: g.id, description: g.description })) : [],
					},
					showtimes: [],
				});
			}
			...
			const cinema = room.cinema ? cinemaMap.get(room.cinema) : null;
			...
			billboardMap.get(movie.id).showtimes.push({
				...
				booking: {
					...
					room: { id: room.id, name: room.name, cinema: cinema ? { id: cinema.id, name: cinema.name } : null },
				},
	...
	//  Mismo patrón que getBillboard pero filtrando por special_event_id NOT NULL
	// -------------------------------------------------------------------------
	async getEventsBillboard(cinemaId?: number) {
		...
		if (cinemaId) {
			bookingList = bookingList.filter((b: any) => Number(b._Rooms?.cinema) === cinemaId);
		}
		...
		if (cinemaIds.length > 0) {
			...
			cinemaMap = new Map((Array.isArray(cinemas) ? cinemas : cinemas.rows || []).map((c: any) => [c.id, c]));
		}

		const bookingIds = bookingList.map((b: any) => b.id);
		// Solo showtimes de eventos especiales (special_event_id NOT NULL)
			...
			if (!billboardMap.has(event.id)) {
				billboardMap.set(event.id, {
					event: {
						...
						lifecycle: event._LifecycleStates ? { id: event._LifecycleStates.id, description: event._LifecycleStates.description } : null,
						age_classification: event._AgeClassifications ? { id: event._AgeClassifications.id, description: event._AgeClassifications.description } : null,
					},
					showtimes: [],
				});
			}
	...
	// -------------------------------------------------------------------------
	//  FUNCIONES DE UNA PELÍCULA EN UNA SUCURSAL (sin cambios)
	// -------------------------------------------------------------------------
	async getMovieShowtimesByCinema(movieId: number, cinemaId: number) {
		...
		const bookings = await this._roomBookings.getAll(
			{
				count: false,
				attributes: ['id', 'room', 'start_time', 'end_time'],
				relations: [{ association: '_Rooms', attributes: ['id', 'name', 'cinema', 'grid_rows', 'grid_columns'], required: true, where: { cinema: cinemaId } }],
		...
		if (!bookingList.length) throw new NotFoundError(`No hay funciones disponibles para esta película en la sucursal seleccionada`);

		const bookingIds = bookingList.map((b: any) => b.id);
		const showtimes = await this._showtimesRepo.getAll(
			{ count: false, attributes: ['id', 'booking', 'movie', 'projection_type', 'language', 'currency', 'price', 'earned_loyalty_points'] },
			{ booking: bookingIds, movie: movieId, deleted_at: null },
		);

		const showtimeList: any[] = Array.isArray(showtimes) ? showtimes : showtimes.rows;
		if (!showtimeList.length) throw new NotFoundError(`No hay funciones disponibles para esta película en la sucursal seleccionada`);

		const movie = await this._movies.getById(movieId, { attributes: ['id', 'title', 'duration_minutes', 'poster_url', 'lifecycle_state'] });
		if (!movie || !visibleStates.includes(movie.lifecycle_state)) throw new NotFoundError('Película no encontrada o no está en cartelera');
			...
			showtimeList.map(async (s: any) => {
				...
				return {
					...
					booking: {
						...
						start_time: booking?.start_time,
						end_time: booking?.end_time,
						room: { id: room.id, name: room.name, total_seats: totalSeats, available_seats: Math.max(0, totalSeats - soldSeats) },
					},
					projection_type: { id: proj.id, description: proj.description },
					language: { id: lang.id, description: lang.description },
					currency: { id: curr.id, code: curr.code, symbol: curr.symbol },
		...
		return {
			movie: { id: movie.id, title: movie.title, duration_minutes: movie.duration_minutes, poster_url: movie.poster_url },
			count: rows.length,
			rows,
		};
	}
	...
	//  Mismo patrón que getMovieShowtimesByCinema
	// -------------------------------------------------------------------------
	async getEventShowtimesByCinema(eventId: number, cinemaId: number) {
		...
		const bookingList: any[] = Array.isArray(bookings) ? bookings : bookings.rows;
		if (!bookingList.length) throw new NotFoundError(`No hay funciones disponibles para este evento en la sucursal seleccionada`);

		const bookingIds = bookingList.map((b: any) => b.id);
		const showtimes = await this._showtimesRepo.getAll(
			{ count: false, attributes: ['id', 'booking', 'special_event_id', 'projection_type', 'language', 'currency', 'price', 'earned_loyalty_points'] },
			{ booking: bookingIds, special_event_id: eventId, deleted_at: null },
		);

		const showtimeList: any[] = Array.isArray(showtimes) ? showtimes : showtimes.rows;
		if (!showtimeList.length) throw new NotFoundError(`No hay funciones disponibles para este evento en la sucursal seleccionada`);

		const event = await this._specialEvents.getById(eventId, { attributes: ['id', 'title', 'duration_minutes', 'poster_url', 'lifecycle_state'] });
		if (!event || !visibleStates.includes(event.lifecycle_state)) throw new NotFoundError('Evento especial no encontrado o no está en cartelera');
		...
		return {
			event: { id: event.id, title: event.title, duration_minutes: event.duration_minutes, poster_url: event.poster_url },
			count: rows.length,
			rows,
		};
	}
	...
	// -------------------------------------------------------------------------
	//  CREAR FUNCIÓN
	//  NUEVO CAMPO: showtime_type = "movie" | "event"
	//  Si no viene, se asume "movie" para retrocompatibilidad.
	// -------------------------------------------------------------------------
	async createShowtime(data: any) {
		...
		if (showtimeType === 'movie') {
			return this._createMovieShowtime({ roomId, projTypeId, languageId, start, end, currencyId, price, earnedLoyaltyPoints, data });
		} else {
			return this._createEventShowtime({ roomId, projTypeId, languageId, start, end, currencyId, price, earnedLoyaltyPoints, data });
		}
	}
	...
	private async _createMovieShowtime(params: any) {
		...
		if (!movieId) throw new ValidationError('El campo movie es obligatorio para funciones de tipo "movie"', ['movie']);

		const [movie, room, language, currency] = await Promise.all([
			this._movies.getById(movieId, { attributes: ['id', 'title', 'duration_minutes', 'release_date', 'lifecycle_state'] }),
			this._rooms.getById(roomId, { attributes: ['id', 'name'] }),
			(Database.repository('main', 'languages') as any).getById(languageId, { attributes: ['id', 'description'] }),
			(Database.repository('main', 'currencies') as any).getById(currencyId, { attributes: ['id', 'code', 'symbol'] }),
		]);
		...
		const releaseDate = new Date(movie.release_date);
		const releaseDateStart = new Date(Date.UTC(releaseDate.getUTCFullYear(), releaseDate.getUTCMonth(), releaseDate.getUTCDate(), 0, 0, 0));
		...
		const roomProjectionType = await (Database.repository('main', 'room-projection-types') as any).getOne({ room: roomId, projection_type: projTypeId });
		if (!roomProjectionType) {
			const projType = await (Database.repository('main', 'projection-types') as any).getById(projTypeId, { attributes: ['id', 'description'] });
			throw new ValidationError(
				`La sala "${room.name}" no admite el tipo de proyección "${projType?.description ?? `id ${projTypeId}`}". Verificá los tipos habilitados para esta sala.`,
			);
		}
		...
		return this._roomBookings.transaction(async (transaction: Transaction) => {
			...
			const showtime = await this._showtimesRepo.create(
				{ booking: booking.id, movie: movieId, special_event_id: null, projection_type: projTypeId, language: languageId, currency: currencyId, price, earned_loyalty_points: earnedLoyaltyPoints },
				{ transaction },
			);
			...
			try {
				const subscriptions = await this._movieSubscriptions.getAll({ count: false }, { movie: movieId, is_notified: false });
				const subList = Array.isArray(subscriptions) ? subscriptions : subscriptions.rows || [];
				for (const sub of subList) await this._movieSubscriptions.update(sub.id, { is_notified: true }, { transaction });
			} catch { /* No interrumpir */ }

			const projType = await (Database.repository('main', 'projection-types') as any).getById(projTypeId, { attributes: ['id', 'description'], transaction });
	...
	private async _createEventShowtime(params: any) {
		...
		if (!eventId) throw new ValidationError('El campo special_event_id es obligatorio para funciones de tipo "event"', ['special_event_id']);

		const [event, room, language, currency, projType] = await Promise.all([
			this._specialEvents.getById(eventId, { attributes: ['id', 'title', 'duration_minutes', 'release_date', 'lifecycle_state'] }),
			this._rooms.getById(roomId, { attributes: ['id', 'name'] }),
			(Database.repository('main', 'languages') as any).getById(languageId, { attributes: ['id', 'description'] }),
			(Database.repository('main', 'currencies') as any).getById(currencyId, { attributes: ['id', 'code', 'symbol'] }),
			(Database.repository('main', 'projection-types') as any).getById(projTypeId, { attributes: ['id', 'description'] }),
		]);
		...
		// Los eventos especiales NO validan room_projection_types:
		// la sala solo necesita estar disponible en el horario solicitado.

		return this._roomBookings.transaction(async (transaction: Transaction) => {
			await this._checkOverlap(Number(roomId), start, end, undefined, transaction);

			let bookingType = await this._bookingTypes.getOne({ description: 'Evento Alternativo' }, { transaction });
			if (!bookingType) bookingType = await this._bookingTypes.getById(BOOKING_TYPE_ID_SPECIAL_EVENT, { transaction });
			if (!bookingType) throw new ValidationError('Falta el tipo de reserva "Evento Alternativo" en booking_types');
			...
			const showtime = await this._showtimesRepo.create(
				{ booking: booking.id, movie: null, special_event_id: eventId, projection_type: projTypeId, language: languageId, currency: currencyId, price, earned_loyalty_points: earnedLoyaltyPoints },
				{ transaction },
			);
	...
	// -------------------------------------------------------------------------
	//  LISTAR FUNCIONES (ADMIN) - extendido para incluir eventos
	// -------------------------------------------------------------------------
	async findAllShowtimes(filters?: any) {
		const { date, startDate, endDate, cinemaId, movieId, eventId, onlyFuture = true, onlyEvents = false, ...rest } = filters ?? {};
		const now = new Date();

		const bookingWhere: any = { deleted_at: null };
		if (onlyFuture) bookingWhere.end_time = { [Ops.gt]: now };

		if (startDate && endDate) {
			const start = new Date(startDate);
			const end = new Date(endDate);
			if (isNaN(start.getTime()) || isNaN(end.getTime())) throw new ValidationError('startDate y endDate deben ser fechas válidas (YYYY-MM-DD)');
			...
			if (isNaN(dayStart.getTime())) throw new ValidationError('La fecha proporcionada no es válida (YYYY-MM-DD)');
		...
		if (cinemaId || date || startDate || onlyFuture) {
			...
			if (cinemaId) bookingList = bookingList.filter((b: any) => b._Rooms?.cinema === Number(cinemaId));

			targetBookingIds = bookingList.map((b: any) => b.id);
			if (targetBookingIds.length === 0) {
				if (date) throw new NotFoundError(`No hay funciones disponibles para el día ${date}`);
		...
		// onlyEvents=true: solo funciones de eventos especiales (excluye películas)
		if (onlyEvents) showtimeWhere.special_event_id = eventId
			? eventId
			: { [Ops.ne]: null };
		// Sin onlyEvents y sin movieId/eventId explícito: devuelve todo (películas + eventos)

		const rawResult = await this._showtimesRepo.getAll(
			{ ...rest, count: true, attributes: ['id', 'booking', 'movie', 'special_event_id', 'projection_type', 'language', 'currency', 'price', 'earned_loyalty_points'] },
			showtimeWhere,
		);
		...
		const [bookings, movies, events, projTypes, languages, currencies] = await Promise.all([
			bookingIds.length ? this._roomBookings.getAll({ count: false, attributes: ['id', 'room', 'start_time', 'end_time'], relations: [{ association: '_Rooms', attributes: ['id', 'name'] }] }, { id: bookingIds }) : [],
			movieIds.length ? (Database.repository('main', 'movies') as any).getAll({ count: false, attributes: ['id', 'title', 'duration_minutes'] }, { id: movieIds }) : [],
			eventIds.length ? (Database.repository('main', 'special-events') as any).getAll({ count: false, attributes: ['id', 'title', 'duration_minutes'] }, { id: eventIds }) : [],
			projTypeIds.length ? (Database.repository('main', 'projection-types') as any).getAll({ count: false, attributes: ['id', 'description'] }, { id: projTypeIds }) : [],
			languageIds.length ? (Database.repository('main', 'languages') as any).getAll({ count: false, attributes: ['id', 'description'] }, { id: languageIds }) : [],
			currencyIds.length ? (Database.repository('main', 'currencies') as any).getAll({ count: false, attributes: ['id', 'code', 'symbol'] }, { id: currencyIds }) : [],
		]);

		const bookingMap = new Map<number, any>((Array.isArray(bookings) ? bookings : bookings.rows || []).map((b: any) => [b.id, { ...b, _roomName: b._Rooms?.name ?? null }]));
		const movieMap = new Map<number, any>((Array.isArray(movies) ? movies : movies.rows || []).map((m: any) => [m.id, m]));
		const eventMap = new Map<number, any>((Array.isArray(events) ? events : events.rows || []).map((e: any) => [e.id, e]));
		const projMap = new Map<number, any>((Array.isArray(projTypes) ? projTypes : projTypes.rows || []).map((p: any) => [p.id, p]));
		const langMap = new Map<number, any>((Array.isArray(languages) ? languages : languages.rows || []).map((l: any) => [l.id, l]));
		const currencyMap = new Map<number, any>((Array.isArray(currencies) ? currencies : currencies.rows || []).map((c: any) => [c.id, c]));
	...
	// -------------------------------------------------------------------------
	//  DETALLE DE UNA FUNCIÓN - extendido para eventos
	// -------------------------------------------------------------------------
	async findShowtimeById(id: number) {
		if (!id || isNaN(Number(id))) throw new ValidationError('El ID de la función proporcionado no es válido.');
		...
		const [booking, projType, language, currency] = await Promise.all([
			...
			(Database.repository('main', 'projection-types') as any).getById(raw.projection_type, { attributes: ['id', 'description'] }),
			(Database.repository('main', 'languages') as any).getById(raw.language, { attributes: ['id', 'description'] }),
			(Database.repository('main', 'currencies') as any).getById(raw.currency, { attributes: ['id', 'code', 'symbol'] }),
		]);

		const base: any = {
			id: raw.id,
			showtime_type: isEvent ? 'event' : 'movie',
			booking: { id: booking?.id, room: booking?.room, start_time: booking?.start_time, end_time: booking?.end_time },
			projection_type: { id: projType?.id, description: projType?.description },
			language: { id: language?.id, description: language?.description },
			currency: { id: currency?.id, code: currency?.code, symbol: currency?.symbol },
			price: raw.price,
		...
		if (isEvent) {
			const event = await this._specialEvents.getById(raw.special_event_id, { attributes: ['id', 'title', 'duration_minutes'] });
			base.event = { id: event?.id, title: event?.title, duration_minutes: event?.duration_minutes };
		} else {
			const movie = await this._movies.getById(raw.movie, { attributes: ['id', 'title', 'duration_minutes'] });
			base.movie = { id: movie?.id, title: movie?.title, duration_minutes: movie?.duration_minutes };
		}
	...
	// -------------------------------------------------------------------------
	//  ACTUALIZAR FUNCIÓN - detecta automáticamente si es película o evento
	// -------------------------------------------------------------------------
	async updateShowtime(id: number, body: any) {
		const showtime = await this._showtimesRepo.getOne(
			{ id },
			{ attributes: ['id', 'booking', 'movie', 'special_event_id', 'projection_type', 'language', 'currency', 'price', 'earned_loyalty_points'] },
		);
		...
		if (!hasBookingChanges && Object.keys(updateData).length === 0) throw new ValidationError('No se proporcionaron datos para actualizar');
		...
		if (hasBookingChanges) {
			...
			await this._roomBookings.transaction(async (transaction: Transaction) => {
				...
				await this._roomBookings.update(booking.id, { room: newRoom, start_time: newStart, end_time: newEnd }, { transaction });
				if (Object.keys(updateData).length > 0) await this._showtimesRepo.update(id, updateData, { transaction });
	...
	// -------------------------------------------------------------------------
	//  ELIMINAR FUNCIÓN - detecta automáticamente si es película o evento
	// -------------------------------------------------------------------------
	async deleteShowtime(id: number) {
		...
		if (ticketCount > 0) throw new ConflictError('No se puede cancelar la función porque tiene boletos vendidos', 'SHOWTIME_HAS_TICKETS');
	...
	// -------------------------------------------------------------------------
	//  MAPA DE ASIENTOS (sin cambios)
	// -------------------------------------------------------------------------
	async getSeatMap(showtimeId: number) {
		const showtime = await this._showtimesRepo.getOne(
			{ id: showtimeId },
			{ attributes: ['id', 'booking', 'movie', 'special_event_id', 'projection_type', 'language', 'currency', 'price'] },
		);
		if (!showtime) throw new NotFoundError('Función no encontrada');

		const booking = await this._roomBookings.getById(showtime.booking, { attributes: ['id', 'room', 'start_time', 'end_time'] });
		if (!booking) throw new NotFoundError('Reserva de sala no encontrada');

		const allSeats = await this._seats.getAll(
			{ count: false, attributes: ['id', 'row_identifier', 'column_number', 'seat_category', 'seat_condition'], order: [['row_identifier', 'ASC'], ['column_number', 'ASC']] },
			{ room: booking.room },
		);

		if (!allSeats || allSeats.length === 0) {
			return { showtime_id: showtimeId, booking_id: booking.id, room_id: booking.room, start_time: booking.start_time, end_time: booking.end_time, seats: [], summary: { total: 0, available: 0, locked: 0, sold: 0, maintenance: 0 } };
		}

		const soldTickets = await this._tickets.getAll({ count: false, attributes: ['seat'] }, { booking: showtime.booking, deleted_at: null });
		const soldSeatIds = new Set<number>((Array.isArray(soldTickets) ? soldTickets : soldTickets).map((t: any) => t.seat));

		const lockedSeatIds = new Set<number>();
		try {
			const redis = (await import('@providers/cache-database.provider.js')).CacheDatabaseProvider.getInstance().client;
			const lockKeys = await redis.keys(`lock:booking:${booking.id}:seat:*`);
			for (const key of lockKeys) {
				const seatId = Number(key.split(':').pop());
				if (!isNaN(seatId)) lockedSeatIds.add(seatId);
			}
		} catch { /* Ignorar si Redis no está disponible */ }

		const categoryIds = [...new Set(allSeats.map((s: any) => s.seat_category))];
		const categoriesRaw = await (Database.repository('main', 'seat-categories') as any).getAll({ count: false, attributes: ['id', 'description'] }, { id: categoryIds });
		const categoryMap = new Map<number, any>((Array.isArray(categoriesRaw) ? categoriesRaw : categoriesRaw.rows).map((c: any) => [c.id, c]));

		const seats = allSeats.map((seat: any) => {
			let status: 'available' | 'locked' | 'sold' | 'maintenance';
			if (seat.seat_condition !== 1) status = 'maintenance';
			else if (soldSeatIds.has(seat.id)) status = 'sold';
			else if (lockedSeatIds.has(seat.id)) status = 'locked';
			else status = 'available';
			const category = categoryMap.get(seat.seat_category) ?? {};
			return { id: seat.id, row: seat.row_identifier, column: seat.column_number, label: `${seat.row_identifier}${seat.column_number}`, category: { id: category.id ?? null, description: category.description ?? null }, status };
		});
		...
		return { showtime_id: showtimeId, booking_id: booking.id, room_id: booking.room, start_time: booking.start_time, end_time: booking.end_time, summary, seats };
	}

	// -------------------------------------------------------------------------
	//  BILLBOARD CON FILTROS AVANZADOS (sin cambios)
	// -------------------------------------------------------------------------
	async getBillboardFiltered(filters: { cinemaId?: number; movieId?: number; projectionType?: string | number; language?: string | number } = {}) {
		...
		if (filters.movieId) {
			rows = rows.filter((entry: any) => entry.movie?.id === Number(filters.movieId));
		}
		if (filters.projectionType && filters.projectionType !== '') {
			const pt = String(filters.projectionType).toLowerCase();
			rows = rows
				.map((entry: any) => ({ ...entry, showtimes: entry.showtimes.filter((s: any) => (s.projection_type?.description ?? '').toLowerCase().includes(pt) || String(s.projection_type?.id ?? '') === pt) }))
				.filter((entry: any) => entry.showtimes.length > 0);
		}
		if (filters.language && filters.language !== '') {
			const lang = String(filters.language).toLowerCase();
			rows = rows
				.map((entry: any) => ({ ...entry, showtimes: entry.showtimes.filter((s: any) => (s.language?.description ?? '').toLowerCase().includes(lang) || String(s.language?.id ?? '') === lang) }))
				.filter((entry: any) => entry.showtimes.length > 0);
		}

		if (rows.length === 0) throw new NotFoundError('No se encontraron funciones que coincidan con los filtros avanzados seleccionados');
