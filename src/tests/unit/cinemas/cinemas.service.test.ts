...
describe('CinemasService Unit Suite', () => {
    ...
    describe('createCinema', () => {
        it('should throw ValidationError if required fields are missing', async () => {
            await expect(CinemasService.createCinema({ name: '', openingTime: '10:00', closingTime: '22:00' })).rejects.toThrow(ValidationError);
        });

        it('should throw ValidationError if time format is invalid', async () => {
            await expect(CinemasService.createCinema({ name: 'Cine', openingTime: '10:0', closingTime: '22:00' })).rejects.toThrow(ValidationError);
        });
        ...
        it('should throw ConflictError if cinema name already exists and is active', async () => {
            ...
            await expect(CinemasService.createCinema({ name: 'Cineplex', openingTime: '10:00', closingTime: '22:00' })).rejects.toThrow(ConflictError);
        });
        ...
        it('should create cinema and audit log successfully', async () => {
            ...
            const result = await CinemasService.createCinema({ name: 'Cineplex', openingTime: '10:00', closingTime: '22:00' });
    ...
    describe('updateCinema', () => {
        ...
        it('should update cinema successfully', async () => {
            mockCinemas.getFull!.mockResolvedValueOnce({ id: 1, name: 'Old', status: 1, opening_time: '10:00', closing_time: '22:00' });
            mockCinemas.getByName!.mockResolvedValueOnce(null);
