...
class CinemaShowtimesController extends ControllerBase {
    ...
    async getBillboardByCinema() {
        ...
        if (data.count === 0) {
            const cinemaName = (data as any).cinemaName;
            if (cinemaName) {
                return this.success(data, `No hay funciones disponibles en la sucursal ${cinemaName}`);
            }
    ...
    async getMovieShowtimes() {
        ...
        const data = await ShowtimeManagementService.getMovieShowtimesByCinema(
            Number(movieId),
            Number(cinemaId),
            session?.userId,
        );
    ...
    async getSeatMap() {
        const { id } = this.getParams();
        const session = this.getSession();
        const data = await ShowtimeManagementService.getSeatMap(Number(id), session?.userId);
