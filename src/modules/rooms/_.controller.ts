...
class RoomsController extends ControllerBase {
	...
	async findAll() {
		...
		return this.success(data, 'Salas obtenidas exitosamente');
	}
	...
	async findProjectionTypes() {
		const { id } = this.getParams();
		const data = await RoomsService.findRoomProjectionTypes(Number(id));
		return this.success(data, 'Tipos de proyección obtenidos exitosamente');
	}
	...
	async createSeat() {
		const { id } = this.getParams();
		const body = this.getBody();
		const data = await SeatManagementService.createSeats(Number(id), body);
		return this.created(data, 'Asiento(s) agregado(s) exitosamente');
	}
}
