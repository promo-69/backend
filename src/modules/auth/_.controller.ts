...
class AuthController extends ControllerBase {
	...
	private _usesCookieTransport(): boolean {
		const req = this.getRequest();
		const security = AppConfig.load().security;
		const refreshName = security.jwtCookieRefreshName || 'RT';

		return Boolean(req.cookies?.[refreshName]);
	}

	private _usesBearerTransport(): boolean {
	...
	private _getExpectedTransport(): 'cookie' | 'bearer' {
		if (this._usesCookieTransport()) return 'cookie';
		if (this._usesBearerTransport()) return 'bearer';
	...
	private _sendLoginResponse(loginResponse: { user: any; accessToken: string; refreshToken: string }) {
		...
		this.getResponse().setHeader('Authorization', `Bearer ${accessToken}`);
