...
export class EmployeesService extends BaseService {
	...
	async createEmployee(employeeData: any, session: any) {
		...
		if (employeeData.email && employeeData.password) {
			...
			const existingUserByEmail = await this._users.getOne({ email: employeeData.email });
			if (existingUserByEmail) {
				throw new ValidationError(
					'El empleado puede ser creado exitosamente, pero las credenciales para crear su cuenta en sistema no porque ya existen. Omita las credenciales o use distintas.',
				);
			}
		}
