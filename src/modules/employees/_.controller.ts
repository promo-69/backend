...
class EmployeesController extends ControllerBase {
	...
	async findAll() {
		...
		const GLOBAL_ROLES = ['SUPER_ADMIN', 'GENERAL_MANAGER'];
		const cinemaId = GLOBAL_ROLES.includes(session.roleCode) ? undefined : session.cinemaId;

		const data = await EmployeesService.findAllEmployees(cinemaId, this.getQueryFilters());
