...
export class ConcessionsService extends BaseService {
	...
	private parseComboProducts(input: string | Array<{ productId: number; quantity: number }> | undefined) {
		if (input === undefined) return [];

		const products = typeof input === 'string' ? this.safeParseProducts(input) : input;

		if (!Array.isArray(products)) throw new ValidationError('El campo products debe ser un arreglo', ['products']);
		if (products.length === 0) throw new ValidationError('El combo debe tener al menos un producto', ['products']);

		products.forEach((item, index) => {
			if (!item || typeof item.productId !== 'number' || item.productId <= 0)
				throw new ValidationError(`El ítem ${index} debe tener productId válido`, ['products']);
			if (!item.quantity || typeof item.quantity !== 'number' || item.quantity <= 0)
				throw new ValidationError(`El ítem ${index} debe tener quantity positivo`, ['products']);
		});

		return products;
	}

	private safeParseProducts(value: string) {
		try {
			return JSON.parse(value);
		} catch {
			throw new ValidationError('El campo products debe ser un JSON válido', ['products']);
		}
	}
	...
	async createCombo(body: CreateComboBody, rawFiles?: RawFiles) {
		...
		const earnedLoyaltyPoints =
			body.earnedLoyaltyPoints !== undefined ? Number(body.earnedLoyaltyPoints) : undefined;
		const products = this.parseComboProducts(body.products);
	...
	async updateCombo(
		...
		return this._combos.transaction(async (transaction: Transaction) => {
			...
			const products: Array<{ productId: number; quantity: number }> | undefined =
				safeBody.products === undefined ? undefined : this.parseComboProducts(safeBody.products as any);
