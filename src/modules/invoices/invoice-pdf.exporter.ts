...
const fmtDate = (d: Date | string) => new Date(d).toLocaleString('es-VE', { timeZone: 'America/Caracas' });
...
export class InvoicePDFExporter {
	static async toPDF(invoice: any): Promise<Buffer> {
		...
		if (invoice.order?.created_at) {
			doc.fontSize(8)
				.fillColor(GRAY)
				.text(`Orden #${invoice.order.id} — ${fmtDate(invoice.order.created_at)}`, ML + 8, y + 39, {
					width: halfW,
					lineBreak: false,
				});
		}
