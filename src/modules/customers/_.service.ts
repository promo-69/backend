...
export class CustomersService extends BaseService {
	...
	async applyLoyaltyPointsDelta(
		...
		const currentProgress = customer.level_progress_points ?? 0;
		// El progreso de nivel es absoluto: solo incrementa. Un ajuste negativo afecta el
		// saldo gastable (ledger), no el progreso ni el nivel alcanzado.
		const newProgress = delta > 0 ? currentProgress + delta : currentProgress;

		const currentLevelId = customer.loyalty_level ?? 1;
		const computedLevelId = await this._calculateLoyaltyLevel(newProgress, transaction);
		// Nunca se degrada de nivel: se conserva el mayor entre el actual y el calculado.
		...
		if (newBalance < 0) {
			throw new ValidationError('El saldo de puntos resultante no puede ser negativo', ['points']);
		}
